...
public class EventHandler extends GenericEventHandler {
	...
	public EventHandler(JDA jda)
	{
		...
		Main.server.getServer(jda).getTextChannelById(Main.server.getVotingBooth()).getHistoryFromBeginning(100).submit().thenAccept(history ->
		{
	...
	private void slashCommand(SlashCommandInteractionEvent event)
	{
		...
		if(!Main.inIDE && event.getChannel().getIdLong() == Main.server.getTestChannel())
		{
		...
		switch(event.getName())
		{
			...
			case "naturalize":
			{
				...
				Main.server.beginPoll(event, new NaturalizePoll(jda.getTextChannelById(Main.server.getVotingBooth()), member));
				break;
			}
			case "campaign":
			{
				...
				else
				{
					...
					event.reply("You are now campaigning! Check the Presidential Voting poll in <#" + Main.server.getVotingBooth() + ">.").queue();
				}
			...
			case "refer":
			{
				...
				if(number < 1 || number > Main.server.getTotalAmendments())
				{
					event.reply("Enter a number between 1 - " + Main.server.getTotalAmendments()).setEphemeral(true).queue();
					break;
				}
			...
			case "propose":
			{
				...
				// Add the poll
				Main.server.beginPoll(event, new ProposePoll(jda.getTextChannelById(Main.server.getVotingBooth()), content));
				break;
			}
			case "repeal":
			{
				...
				// Add the poll
				Main.server.beginPoll(event, new RepealPoll(jda.getTextChannelById(Main.server.getVotingBooth()), number));
				break;
			}
			case "impeach":
			{
				// Don't allow impeachment if it will overlap with the presidential vote
				ImpeachPoll poll = new ImpeachPoll(jda.getTextChannelById(Main.server.getVotingBooth()), event.getOption("reason").getAsString());
